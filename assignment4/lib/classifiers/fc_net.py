...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_loss =  self.reg * np.sum(W1**2)
        reg_loss += self.reg * np.sum(W2**2)
...
class FullyConnectedNet(object):
      ...
      def loss(self, X, y=None):
        ...
        for i in xrange(1, self.num_layers):
          scores[i], cache[i] = affine_relu_forward(scores[i - 1],
                                                         self.params['W%d' % i],
                                                         self.params['b%d' % i])
        # Forward into last layer
        WLast = 'W%d' % self.num_layers
        bLast = 'b%d' % self.num_layers
        scores, cache_scores = affine_forward(scores[self.num_layers - 1],
                                              self.params[WLast],
                                              self.params[bLast])
        ############################################################################
        #                             END OF YOUR CODE                             #
